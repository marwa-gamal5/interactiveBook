...
const AnnotationTools = ({ canvasId }) => {
  ...
  useEffect(() => {
    if (!editor || !fabric || !editor.canvas) {
      return;
    }
  ...
  const saveToLocalStorage = (canvasId) => {
    const canvasData = JSON.stringify(editor.canvas.toObject());
    localStorage.setItem(`fabricCanvas_${canvasId}`, canvasData);
  };

  const loadFromLocalStorage = (canvasId) => {
    const savedCanvasData = localStorage.getItem(`fabricCanvas_${canvasId}`);
    if (savedCanvasData) {
  ...
  useEffect(() => {
    ...
    loadFromLocalStorage(canvasId)
  }, [editor]);
  return (
    <>
      <div >
        <div className={`${styles.AnnotationTool} ms-5 d-flex justify-content-around align-items-center px-5  `}>
